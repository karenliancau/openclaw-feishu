  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "POST" && req.url === "/query") {
      ...
      req.on("end", async () => {
        ...
        let streamed = "";
        const pushTextDelta = (full: string) => {
        ...
        try {
          await channel.reply.dispatchReplyWithBufferedBlockDispatcher({
            ...
            replyOptions: {
              onPartialReply: (p: { text?: string; delta?: string }) => {
                if (p?.delta) pushTextDelta(streamed + p.delta);
                else if (typeof p?.text === "string") pushTextDelta(p.text);
              },
              onReasoningStream: (p: { text?: string }) => {
                if (p?.text) emit({ type: "thinking_delta", text: p.text });
              },
              onToolStart: (p: {
                toolCallId?: string;
                name?: string;
