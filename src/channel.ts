...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount> = {
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as ClawdbotConfig).channels?.[FEISHU_CHANNEL_ID]?.accounts?.[resolvedAccountId],
      );
  ...
  setup: {
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const feishuCfg = (next.channels?.[FEISHU_CHANNEL_ID] ?? {}) as Record<string, unknown>;
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      ctx.log?.info(
        `[${account.accountId}] Starting OpenClaw HTTP bridge${feishuBotLabel}`,
      );

      const server = startChatServer(
        ctx.cfg as ClawdbotConfig,
        account.accountId,
        {
