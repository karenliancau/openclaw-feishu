...
export async function startFeishuProvider(
  ...
  try {
    ...
    // Get bot info
    const botData: any = await new Promise((resolve, reject) => {
  ...
  const dispatcher = new Lark.EventDispatcher({}).register({
    "im.message.receive_v1": async (data: Record<string, unknown>) => {
      try {
        ...
        log.error(
          `[${FEISHU_CHANNEL_ID}:${account.accountId}] Message handler error: ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    },
  });
...
async function handleIncomingMessage(
  ...
  ctx.log.info(`[${FEISHU_CHANNEL_ID}:${ctx.account.accountId}] Incoming message type=${messageType} id=${messageId} content=${((message.content as string) ?? "").slice(0, 200)}`);
  const supportedTypes = ["text", "file", "post", "image"];
  if (!messageType || !supportedTypes.includes(messageType) || !message.content) return;

  let text: string;
  try {
    const parsed: any = JSON.parse(message.content as string);
    if (messageType === "text") {
  ...
  if (chatType === "group") {
    const mentions: any[] = Array.isArray(message.mentions) ? (message.mentions as any[]) : [];
    const botMentioned = mentions.some((m) => {
      if (ctx.botOpenId && m.id?.open_id === ctx.botOpenId) return true;
      return false;
    });
  ...
  try {
      ...
      dispatcherOptions: {
        deliver: async (payload: unknown) => {
          ...
          try {
            ...
            ctx.log.error(
              `[${FEISHU_CHANNEL_ID}:${ctx.account.accountId}] Failed to send reply: ${err instanceof Error ? err.message : String(err)}`,
            );
          }
        },
        onError: (err: Error) => {
    ...
    if (timer) clearTimeout(timer);
    ctx.log.error(
      `[${FEISHU_CHANNEL_ID}:${ctx.account.accountId}] Dispatch error: ${err instanceof Error ? err.message : String(err)}`,
    );
    if (placeholderId) {
      await deleteMessage(ctx.client, placeholderId);
    }
...
async function downloadAndExtractFeishuFile(
  client: any,
  ...
  try {
    ...
    if (!tenantToken) throw new Error("Failed to obtain tenant_access_token");
    ...
    await new Promise<void>((resolve, reject) => {
      const req = https.default.get(fileUrl, {
        ...
        if (res.statusCode !== 200) {
          let body = ""; res.on("data", (c) => body += c); res.on("end", () => reject(new Error(`HTTP ${res.statusCode}: ${body.slice(0, 200)}`)));
          return;
        }
        const ws = fs.createWriteStream(tempPath);
        res.pipe(ws);
    ...
    if (ext === ".pdf") {
      try {
        ...
        if (typeof pdfParse !== "function") throw new Error("pdf-parse module loaded but is not callable: " + typeof pdfParse + " keys=" + Object.keys(pdfModule).join(","));
        const dataBuffer = fs.readFileSync(tempPath);
        const pdfData: any = await pdfParse(dataBuffer);
        const pdfText = (pdfData.text ?? "").trim();
        if (!pdfText) return `[PDF文件: ${fileName} (${(fileSize / 1024).toFixed(1)}KB)] — 无法提取文字内容（可能是扫描件）`;
...
function extractTextFromPost(parsed: any): string {
  const parts: string[] = [];
