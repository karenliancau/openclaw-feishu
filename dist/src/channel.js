 ...
 */
import { applyAccountNameToChannelSection, buildChannelConfigSchema, DEFAULT_ACCOUNT_ID, deleteAccountFromConfigSection, formatPairingApproveHint, migrateBaseNameToDefaultAccount, normalizeAccountId, PAIRING_APPROVED_MESSAGE, setAccountEnabledInConfigSection, } from "openclaw/plugin-sdk";
...
const meta = {
    ...
    docsPath: "/channels/openclaw-feishu",
    docsLabel: "openclaw-feishu",
...
export const feishuPlugin = {
    ...
    reload: { configPrefixes: [`channels.${FEISHU_CHANNEL_ID}`] },
    ...
    security: {
        resolveDmPolicy: ({ cfg, accountId, account }) => {
            ...
            const useAccountPath = Boolean(cfg.channels?.[FEISHU_CHANNEL_ID]?.accounts?.[resolvedAccountId]);
            const basePath = useAccountPath
                ? `channels.${FEISHU_CHANNEL_ID}.accounts.${resolvedAccountId}.`
                : `channels.${FEISHU_CHANNEL_ID}.`;
            return {
                policy: account.config.dmPolicy ?? "pairing",
                allowFrom: account.config.allowFrom ?? [],
                policyPath: `${basePath}dmPolicy`,
                allowFromPath: basePath,
                approveHint: formatPairingApproveHint(FEISHU_CHANNEL_ID),
    ...
    setup: {
        ...
        applyAccountConfig: ({ cfg, accountId, input }) => {
            ...
            if (accountId === DEFAULT_ACCOUNT_ID) {
                return {
                    ...
                    channels: {
                        ...next.channels,
                        [FEISHU_CHANNEL_ID]: {
                            ...next.channels?.[FEISHU_CHANNEL_ID],
                            enabled: true,
            ...
            const feishuCfg = (next.channels?.[FEISHU_CHANNEL_ID] ?? {});
    ...
    outbound: {
        ...
        sendText: async ({ to, text, accountId, cfg }) => {
            ...
            if (!account.appId || !account.appSecret) {
                return { channel: FEISHU_CHANNEL_ID, ok: false, messageId: "", error: new Error("Feishu credentials not configured") };
            }
            const client = createFeishuClient(account);
