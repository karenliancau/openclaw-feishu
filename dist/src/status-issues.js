...
function readFeishuAccountStatus(value) {
    if (!isRecord(value))
        return null;
    return {
...
export function collectFeishuStatusIssues(accounts) {
    const issues = [];
    for (const entry of accounts) {
        ...
        if (enabled && configured && account.dmPolicy === "open") {
            issues.push({
                ...
                accountId,
                kind: "config",
                message: 'Feishu dmPolicy is "open", allowing any user to message the bot without pairing.',
